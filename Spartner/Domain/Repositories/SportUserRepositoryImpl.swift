...
class SportUserRepositoryImpl: SportUserRepository {
    private let db = Firestore.firestore()

    func getUser(byId id: String) async throws -> SportUser {
        let document = try await db.collection("users").document(id)
            .getDocument()
        guard let data = document.data() else {
            throw NSError(
                domain: "FirestoreError", code: 404,
